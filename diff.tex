...
%DIF DEL old.tex                   Thu Dec 28 18:43:10 2017
%DIF ADD serge_kozlukov_jpcs.tex   Thu Dec 28 20:12:22 2017
\usepackage{graphicx}
\usepackage{iopams}
...
\begin{document}
\title{The method of similar operators in the study of the spectra of \DIFaddbegin \DIFadd{the }\DIFaddend adjacency matrices of graphs}

\author{Serge Kozlukov\DIFaddbegin \\
\DIFadd{Department of Nonlinear Oscillations}\DIFaddend }

\DIFdelbegin %DIFDELCMD < \address{\emph{Voronezh State University}, 1 Universitetskaya Ploshad', Voronezh, RU 394036}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \address{Voronezh State University, 1 Universitetskaya Ploshad', Voronezh, RU 394036}
\DIFaddend 
...
\begin{abstract}
    ...
    Specifically, we consider
        \DIFaddbegin \DIFadd{the adjacency matrix of an ``almost-complete graph''.
    Then we generalize the result to allow the }\DIFaddend matrices obtained as combinations
        of the Kronecker products~\cite{bellman-matrices-kron,XIANG2005210}
        and \DIFaddbegin \DIFadd{the }\DIFaddend small-norm perturbations.
...
\DIFdelbegin \DIFdel{Its adjacency matrix is defined as
    the }\DIFdelend \DIFaddbegin \DIFadd{It is uniquely defined by its
adjacency }\DIFaddend matrix
    \( A = (a_{ij}) \)
    of the size \( N\times N \),
    in which \( a_{ij} \)
    is the number of edges
    from \DIFaddbegin \DIFadd{the }\DIFaddend vertex \( i \)
    to \DIFaddbegin \DIFadd{the }\DIFaddend vertex \( j \).
\DIFdelbegin \DIFdel{One can also consider generalized
    adjacency matrices~\mbox{%DIFAUXCMD
\cite{van2003graphs}
    }\hspace{0pt}%DIFAUXCMD
which are defined as
    arbitrary linear }\DIFdelend \DIFaddbegin \DIFadd{The }\emph{\DIFadd{simple spectrum}} \DIFadd{of a graph
...
It is customary to consider }\DIFaddend combinations
    of the matrices \( A \)\DIFdelbegin \DIFdel{(or its transpose \( A^\mathtt{T} \)}\DIFdelend \DIFaddbegin \DIFadd{,
    \( D \) (the out-degree matrix}\DIFaddend ),
...
For instance, the \DIFdelbegin \DIFdel{markovian }\DIFdelend \DIFaddbegin \DIFadd{Markovian }\DIFaddend random walk on a~graph
    yields the notion of eigenvector centrality
    in a~network~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{bonacich1972factoring}}\hspace{0pt}%DIFAUXCMD
.
The transition matrix of such a random walk
    is generalized adjacency matrix of a graph.
The score of the }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{ilprints422,bonacich1972factoring}}\hspace{0pt}%DIFAUXCMD
.
The score of a }\DIFaddend vertex \( i \)
    is defined as the \( i \)'th coordinate
    of \DIFdelbegin \DIFdel{the dominating left }\DIFdelend \DIFaddbegin \DIFadd{a dominating (left) }\DIFaddend eigenvector
    of the transition matrix \DIFdelbegin \DIFdel{.
The largest left eigenvalue of an irreducible column-stochastic matrix
    is \( 1 \) and the dominating left }\DIFdelend \DIFaddbegin \DIFadd{of such a process.
This }\DIFaddend eigenvector
    defines the only stationary distribution
    of \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{this }\DIFaddend random walk.
...
Stability of \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{a }\DIFaddend stationary distribution
    is determined~\cite{meyer1994sensitivity}
    by the condition number
    which is bounded from below
    by \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend spectral gap --- the distance between
    the two largest eigenvalues
    of the transition matrix \DIFaddbegin \DIFadd{of a process}\DIFaddend .
The method of estimation of almost-invariant sets
    proposed in~\cite{schwartz2006fluctuation}
    \DIFdelbegin \DIFdel{relies as well }\DIFdelend \DIFaddbegin \DIFadd{also relies }\DIFaddend on spectral decompositions of such matrices.
In the Susceptible-Infective-Susceptible model
    a viral spread in a network
    is modeled~\cite{wang2003epidemic,chakrabarti2008epidemic} as a \DIFdelbegin \DIFdel{markov }\DIFdelend \DIFaddbegin \DIFadd{Markov }\DIFaddend process
    with \( 2^N \) states.
An asymptotic (endemic or epidemic) behaviour of \DIFdelbegin \DIFdel{this }\DIFdelend \DIFaddbegin \DIFadd{such a }\DIFaddend system
    is determined by \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend spectral radius (the largest absolute eigenvalue)
    of the adjacency matrix
    and the rates of curage and infection.

We should also note that the Kronecker products
    of \DIFaddbegin \DIFadd{the }\DIFaddend adjacency matrices
    are themselves of interest
    as they correspond to the adjacency matrices of
    \DIFaddbegin \DIFadd{the }\DIFaddend \emph{non-complete extended p-sum}s~(NEPS)
...
\DIFaddbegin \DIFadd{In some applications
    it is infeasible to compute the exact eigenvalues.
Then one needs to make reasonable estimates.
}\DIFaddend We use the abstract method of similar operators
    to estimate the eigenvalues and eigenvectors
    \DIFdelbegin \DIFdel{of certain types of such }\DIFdelend \DIFaddbegin \DIFadd{for a certain class of }\DIFaddend matrices.
...
This approach is often superior to usual methods of perturbation \DIFdelbegin \DIFdel{theory
    }\DIFdelend \DIFaddbegin \DIFadd{analysis
    }\DIFaddend that use series expansions.
\DIFdelbegin \DIFdel{Here }\DIFdelend \DIFaddbegin \DIFadd{We are only concerned with finite-dimensional problems
    so }\DIFaddend we will only state the required notation and theorems
    in a simplified form\DIFdelbegin \DIFdel{that accounts
    for the finite-dimensionality
    of the problems we consider}\DIFdelend .
...
Similar matrices share some spectral properties:
    ...
    and \( U \) maps \DIFaddbegin \DIFadd{the }\DIFaddend eigenvectors of one to another's:
    \( A_2 x = \lambda x \implies A_1 U x = \lambda U x \).

The most important notion
    \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend the abstract method of similar operators
    is that of an \emph{admissible triple}.
For our specific purposes it suffices to say
    that \DIFdelbegin \DIFdel{\( (\mathbb{K}^n, J, \Gamma) \)
    }\DIFdelend \DIFaddbegin \DIFadd{\( (\mathbb{K}^{n{\times}n}, J, \Gamma) \)
    }\DIFaddend forms an \emph{admissible triple}
...
The main theorem of the method
    may be \DIFdelbegin \DIFdel{then }\DIFdelend \DIFaddbegin \DIFadd{now }\DIFaddend formulated as follows:
...
\DIFaddbegin \DIFadd{This example has already been considered in~\mbox{%DIFAUXCMD
\cite{sergekozlukov@volgograd}
}\hspace{0pt}%DIFAUXCMD
...
The Frobenius norm \DIFdelbegin \DIFdel{\( {\|B\|_{\mathrm{F}} = \sqrt{\sum_{ij} b_ij^2} = M} \)
    }\DIFdelend \DIFaddbegin \DIFadd{\( {\|B\|_{\mathrm{F}} = \sqrt{\sum_{ij} b_{ij}^2} = M} \)
    }\DIFaddend of \( B \)
...
Now let \( A\in\mathbb{K}^{M{\times}M} \)
    ...
    \]
    and \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend perturbed matrix
    \[
        \mathbb{A} - \mathbb{B},\ \mathbb{B}\in\mathbb{K}^{{MN}{\times}{MN}}.
    \]

\begin{nkjpcslem}
    Suppose \( A \) is \DIFaddbegin \DIFadd{an }\DIFaddend invertible self-adjoint matrix.
...
Now we consider Kronecker \DIFdelbegin \DIFdel{products
}\DIFdelend \DIFaddbegin \DIFadd{product
}\DIFaddend \[
    A\otimes B =
    \begin{pmatrix}
...
Kronecker product has \DIFdelbegin \DIFdel{quite some }\DIFdelend \DIFaddbegin \DIFadd{several }\DIFaddend appealing properties~\cite{bellman-matrices-kron}.
\begin{itemize}
...
These \DIFdelbegin \DIFdel{eigenspace }\DIFdelend \DIFaddbegin \DIFadd{eigenspaces }\DIFaddend form a direct-sum decomposition of \( \mathbb{K}^{MN} \):
...
Now we should be able \DIFaddbegin \DIFadd{to }\DIFaddend reproduce the same steps as before
...
An example of more general \DIFdelbegin \DIFdel{problem
    }\DIFdelend yet quite similar \DIFdelbegin \DIFdel{to the one just considered
    }\DIFdelend \DIFaddbegin \DIFadd{problem
    }\DIFaddend is that of \DIFaddbegin \DIFadd{an }\DIFaddend analysis of tensor products
    of finite-dimensional operators (defined by matrices)
    and abstract operators in Banach spaces.

\section*{References}
\bibliographystyle{iopart-num}
\DIFdelbegin %DIFDELCMD < \bibliography{sergey_kozlukov_jpcs}{}
%DIFDELCMD <  %%%
\DIFdelend \DIFaddbegin \bibliography{serge_kozlukov_jpcs}{}
